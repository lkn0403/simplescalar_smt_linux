...
void
sim_reg_stats(int thread_num, struct stat_sdb_t *sdb)   /* stats database */
{
  int i;
  ...
  for (int tid = 0; tid < thread_num; tid++) {
    sprintf(threadname[tid], "sim_thread_insn %d", tid);

    stat_reg_counter(sdb, threadname[tid],
        "total simulation time in cycles",
        &sim_thread_insn[tid], /* initial value */0, /* format */NULL);
  }
...
void fetch_push_tail(struct fetch_queue *node) {
  ...
  if (fetch_tail) {
    ...
    node->prev = fetch_tail;
    fetch_tail = node;
  } else {
    node->prev = NULL;
    fetch_data = fetch_tail = node;
  }
  fetch_num++;
}
