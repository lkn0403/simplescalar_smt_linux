...
//          warn("mmap ignorning suggested map address");
          ...
          addr = _system[tid].mmap_end;
          _system[tid].mmap_end += length;
...
//        memory->bcopy(true, vaddr, &buf, sizeof(buf));
	...
	if (new_length > old_length) {
		if ((addr + old_length) == _system[tid].mmap_end) {
			unsigned long int diff = new_length - old_length;
			_system[tid].mmap_end += diff;
			return_value = addr;
			success = true;
			return;
		} else {
//			if (!(flags & 1)) {
		 	...
		 	mem_bcopy(mem_fn, mem, Write,
		  		/*buf*/_system[tid].mmap_end, buf, /*nread*/old_length);
			return_value = _system[tid].mmap_end;
			_system[tid].mmap_end += new_length;			
