...
void initialize_system_state(int tid)
{
  ...
  _system[tid].brk_point = (ld_bss_base[tid] + ld_bss_size[tid] + 0xffffLL) & 0xffffffffffff0000LL;
  
  _system[tid].mmap_end = 0x10000; 
}
...
//  ld_text_base = MD_SWAPQ(phdr.text_start);
  ...
  ld_prog_entry[tid] = fhdr.e_entry;
  ld_text_size[tid] = ld_text_base[tid] = 0;
  ld_data_size[tid] = ld_data_base[tid] = 0;
  ...
  for (i = 0; i < fhdr.e_shnum; i++)
  {
    ...
    {
      ld_text_size[tid] = MD_SWAPQ(shdr.sh_size);
      ld_text_base[tid] = MD_SWAPQ(shdr.sh_addr);
//      ld_prog_entry = shdr.sh_addr;
    }
    // store size and base of data segment
    if (!strcmp(".data", shdr_strs + shdr.sh_name))
    {
      ld_data_size[tid] = MD_SWAPQ(shdr.sh_size);
      ld_data_base[tid] = MD_SWAPQ(shdr.sh_addr);
    }
    // store size and base of bss segment
    if (!strcmp(".bss", shdr_strs + shdr.sh_name))
    {
//      ld_prog_entry = shdr.sh_addr;
      ld_bss_size[tid] = MD_SWAPQ(shdr.sh_size);
      ld_bss_base[tid] = MD_SWAPQ(shdr.sh_addr);
    }
  }
...
//    fseek(fobj, sizeof(struct ecoff_filehdr) + MD_SWAPH(fhdr.f_opthdr), 0);
  ...
  uint64_t stack_base = (ld_text_base[tid] - 1024*1024*8) & 0xFFFFFFFFFFFF0000LL;
  uint64_t stack_min = ld_stack_base[tid];
